...
class Inference:
    ...
    def make_op(self, fn):
        ...
        Returns:
            ...
            """
        if self.n_gpus == 1:
            logger.debug('Running CPU method')
            return compute_parallel(fn, self.inputs_parallel)
        else:
            logger.info("Please write a compute_parallel for CPU - Ended!")
    ...
    def make_session(self, sess=None, checkpoint=None, reuse_session=True):
        logger.info('session variables in inference')
        if sess is not None:
...
def main():
    ...
    parser.add_argument('--checkpoint',type=str, default='./backward/log/checkpoint/model-0')
    parser.add_argument('--batch_capacity', type=int, default=4096)
